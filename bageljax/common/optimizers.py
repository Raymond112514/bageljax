    ...
    def optimizer(learning_rate: float, weight_decay: Optional[float]):
        ...
        if weight_decay is not None:
            optimizer_stages.append(
                optax.adamw(learning_rate=learning_rate, weight_decay=weight_decay, b2=0.999 if beta2 is None else beta2, eps=1.0e-8 if eps is None else eps, mu_dtype=jnp.float32)
            )
        else:
            optimizer_stages.append(optax.adam(learning_rate=learning_rate, b2=0.999 if beta2 is None else beta2, eps=1.0e-8 if eps is None else eps, mu_dtype=jnp.float32))
