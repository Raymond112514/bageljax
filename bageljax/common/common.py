...
from bageljax.common.typing import Params, PRNGKey
...
class TrainState(struct.PyTreeNode):
    ...
    params: Params # stored in bfloat16
    target_params: Params # stored in float32
    txs: Any = struct.field(pytree_node=False)
    opt_states: Any # stored in float32
    rng: PRNGKey
    ...
    @staticmethod
    def cast_to_dtype(pytree, dtype=jnp.float32):
        return jax.tree_util.tree_map(
    ...
    def target_update(self, tau: float) -> "TrainState":
        ...
        """
        params_f32 = self.cast_to_dtype(self.params, dtype=jnp.float32)
        new_target_params = jax.tree_util.tree_map(
            lambda p, tp: p * tau + tp * (1 - tau), params_f32, self.target_params
        )
    ...
    def apply_gradients(self, *, grads: Any) -> "TrainState":
        ...
        updates_and_new_states = self._tx_tree_map(
            lambda tx, opt_state, grad: tx.update(grad, opt_state, params_f32),
            self.txs,
            self.opt_states,
            grads,
        )
        ...
        new_params = optax.apply_updates(params_f32, updates_acc)
        new_params_bf16 = self.cast_to_dtype(new_params, dtype=jnp.bfloat16)

        return self.replace(
            step=self.step + 1, params=new_params_bf16, opt_states=new_opt_states
        )
    ...
    def create(
        ...
        return cls(
            ...
            txs=txs,
            opt_states=cls._tx_tree_map(lambda tx: tx.init(cls.cast_to_dtype(params, dtype=jnp.float32)), txs),
            rng=rng,
        )